...
const transformProps = (
  ...
  const {
    ...
    top_metric,
    top_count = 10,
    sticky_columns: stickyColumns = 0,
    show_row_group_totals: showRowGroupTotals = false,
    allow_row_grouping_change: allowRowGroupingChange = false,
    row_grouping_default_collapsed: rowGroupingDefaultCollapsed = false,
    row_grouping_compact_view: rowGroupingCompactView = false,
  } = formData;
  ...
  const requestedTopMetricLabel = top_metric
    ? getMetricLabel(top_metric)
    : undefined;
  const topMetricLabel = topMetricOptions.some(
    option => option.value === requestedTopMetricLabel,
  )
    ? requestedTopMetricLabel
    : topMetricOptions[0]?.value;

  const withRowNumbers = formData.show_row_numbers
    ? data.map((row, index) => ({ '#': index + 1, ...row }))
    : data;

  const topProcessed =
    show_top && topMetricLabel
      ? processTopData(data, columns, topMetricLabel, top_count)
      : undefined;

  let finalData = topProcessed ?? data;
  ...
  const topConfig =
    show_top && topMetricLabel
      ? {
          enabled: true,
          metric: topMetricLabel,
          defaultCount: top_count,
          allowUserControl: Boolean(formData.top_show_in_chart),
          baseData: data,
          columns,
          showRowNumbers: Boolean(formData.show_row_numbers),
          topMetrics: topMetricOptions,
        }
      : undefined;

  if (!show_top) {
    finalData = withRowNumbers;
  }
  ...
  const safeRowGroupingColumn =
    formData.enable_row_grouping &&
    passedColumns.some(
      column =>
        column.key === formData.row_grouping_column &&
        column.key !== '#' &&
        !column.isMetric &&
        !column.isPercentMetric,
    )
      ? formData.row_grouping_column
      : undefined;
  ...
  const stickyColumnsCount = Math.min(
    3,
    Math.max(0, Number(stickyColumns) || 0),
  );
  const jinjaFieldLabels = ensureIsArray(formData.jinja_fields).map(getMetricLabel);
  const jinjaValues = extractJinjaValues(baseQuery?.data || [], jinjaFieldLabels);
  const rawColumnConfig = (formData.column_config ?? {}) as Record<
    string,
    TableColumnConfig
  >;
  const calculatedColumns = normalizeCalculatedColumns(
    formData.calculated_columns ?? [],
  ).map(
    (column): CalculatedColumnConfig => {
      const config = rawColumnConfig[column.label] || {};
      return {
        ...column,
        displayLabel: resolveJinjaTemplate(
          config.displayName || column.label,
          jinjaValues,
        ),
        config: {
          ...config,
          ...(config.d3NumberFormat || !column.d3format
            ? {}
            : { d3NumberFormat: column.d3format }),
        },
      };
    },
  ) as CalculatedColumnConfig[];
  ...
  return {
    ...
    topConfig,
    stickyColumnsCount,
    rowGroupingColumn: safeRowGroupingColumn,
    allowRowGroupingChange: Boolean(
      safeRowGroupingColumn && allowRowGroupingChange,
    ),
    showRowGroupTotals: Boolean(safeRowGroupingColumn && showRowGroupTotals),
    rowGroupingDefaultCollapsed: Boolean(
      safeRowGroupingColumn && rowGroupingDefaultCollapsed,
    ),
    rowGroupingCompactView: Boolean(
      safeRowGroupingColumn && rowGroupingCompactView,
    ),
    calculatedColumns,
    columnOrder: ensureIsArray(formData.column_order)
