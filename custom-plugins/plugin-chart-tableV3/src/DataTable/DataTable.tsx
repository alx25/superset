...
export interface DataTableProps<D extends object> extends TableOptions<D> {
  ...
  onSortByChange?: (sortBy: Array<SortingRule<D>>) => void;
  onStickyColumnWidthsChange?: (widths: number[]) => void;
  /** Render a full-width header row for grouped data rows. */
...
export default typedMemo(function DataTable<D extends object>({
  ...
  const previousStickyWidths = useRef<number[] | undefined>(undefined);

  useEffect(() => {
    if (!onStickyColumnWidthsChange || !sticky.columnWidths) {
      return;
    }
    if (!isEqual(previousStickyWidths.current, sticky.columnWidths)) {
      previousStickyWidths.current = sticky.columnWidths;
      onStickyColumnWidthsChange(sticky.columnWidths);
    }
  }, [onStickyColumnWidthsChange, sticky.columnWidths]);
      ...
      <tbody {...getTableBodyProps()}>
        {page && page.length > 0 ? (
          page.map(row => {
            ...
            const isOthersRow = original?.__isOthersRow === true;
            const isGroupHeader =
              // eslint-disable-next-line no-underscore-dangle
              original?.__isGroupHeader === true;
            const isGroupedRow =
              // eslint-disable-next-line no-underscore-dangle
              original?.__groupParentKey !== undefined;
            const rowClassName = [
              isOthersRow ? 'dt-others-row' : '',
              isGroupHeader ? 'dt-group-header-row' : '',
              isGroupedRow ? 'dt-grouped-row' : '',
            ]
              .filter(Boolean)
              .join(' ');
            ...
            if (isGroupHeader && renderGroupHeaderRow) {
              return (
                <tr key={rowKey || row.id} {...rowProps} role="row">
                  <td colSpan={columns.length} className="dt-group-header-cell">
                    {renderGroupHeaderRow(original, columns.length)}
                  </td>
                </tr>
              );
            }
