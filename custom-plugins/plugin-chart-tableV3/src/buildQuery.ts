...
const normalizeCalculatedColumnsForExport = (
  raw: TableChartFormData['calculated_columns'],
) => {
  const seen = new Set<string>();
  return ensureIsArray(raw)
    .map(item => {
      const label = String((item as any)?.label ?? '').trim();
      const expression = String((item as any)?.expression ?? '').trim();
    ...
    .filter(Boolean) as Array<{ label: string; expression: string }>;
};
...
const buildQuery: BuildQuery<TableChartFormData> = (
  ...
  return buildQueryContext(formDataCopy, baseQueryObject => {
    ...
    if (queryMode === QueryMode.Aggregate) {
      metrics = metrics || [];
      const selectedMetricLabels = metrics.map(getMetricLabel);
      ...
      // Add top metric to metrics for query if it's not already included
      if (
        topMetric &&
        selectedMetricLabels.includes(getMetricLabel(topMetric)) &&
        !metrics.some(m => getMetricLabel(m) === getMetricLabel(topMetric))
      ) {
        metrics = removeDuplicates(metrics.concat([topMetric]), getMetricLabel);
      }
    ...
    // Build column display names map for CSV/Excel export
    const columnConfig = formDataCopy.column_config || {};
    const columnDisplayNames: Record<string, string> = {};
    Object.entries(columnConfig).forEach(([columnName, config]) => {
      if (config.displayName) {
        columnDisplayNames[columnName] = config.displayName;
      }
    });

    // Add column display names to extras if any exist
    if (Object.keys(columnDisplayNames).length > 0) {
      (extras as any).column_display_names = columnDisplayNames;
    }

    // Add fields that should be excluded from CSV/Excel export
    const excludedColumns: string[] = [];

    // Exclude jinja fields (they're only used for template resolution)
    if (jinjaFields && jinjaFields.length > 0) {
      excludedColumns.push(...jinjaFields.map(getMetricLabel));
    }

    // Add excluded columns to extras if any exist
    if (excludedColumns.length > 0) {
      (extras as any).excluded_columns = excludedColumns;
    }

    const exportCalculatedColumns = normalizeCalculatedColumnsForExport(
      formDataCopy.calculated_columns,
    );
    if (exportCalculatedColumns.length > 0) {
      (extras as any).calculated_columns_export = exportCalculatedColumns;
    }

    const exportColumnOrder = ensureIsArray(formDataCopy.column_order)
      .map(item => String(item ?? '').trim())
      .filter(Boolean);
    if (exportColumnOrder.length > 0) {
      (extras as any).column_export_order = exportColumnOrder;
    }
