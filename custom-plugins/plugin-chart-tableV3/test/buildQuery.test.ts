...
describe('plugin-chart-table', () => {
  describe('buildQuery', () => {
    ...
    it('should include calculated columns and column order for exports', () => {
      const query = buildQuery({
        ...basicFormData,
        query_mode: QueryMode.Aggregate,
        groupby: ['name'],
      ...
      const extras = query.extras as any;

      expect(extras?.calculated_columns_export).toEqual([
        {
          label: 'ratio',
          expression: '{{sum__num}} / {{sum__denom}}',
        },
      ]);
      expect(extras?.column_export_order).toEqual([
        'sum__num',
        'ratio',
        'name',
      ]);
    ...
    it('should keep the selected Top N metric available instead of excluding it', () => {
      const query = buildQuery({
        ...basicFormData,
        query_mode: QueryMode.Aggregate,
        groupby: ['name'],
      ...
      expect(extras?.excluded_columns).toBeUndefined();
      expect(query.metrics).toEqual(['sum__num', 'sum__denom']);
    });
  });
});
