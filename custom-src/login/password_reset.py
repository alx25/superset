...
class PasswordResetView(BaseView):
    ...
    def _password_reset_email_enabled(self) -> bool:
        """Controla si se envían correos de recuperación.

        - `PASSWORD_RESET_EMAIL_ENABLED=False` desactiva explícitamente el envío.
        - Mantiene compatibilidad con instalaciones que usan `EMAIL_NOTIFICATIONS=False`.
        """

        if current_app.config.get("PASSWORD_RESET_EMAIL_ENABLED") is False:
            return False
        if (
            "EMAIL_NOTIFICATIONS" in current_app.config
            and current_app.config.get("EMAIL_NOTIFICATIONS") is False
        ):
            return False
        return True

    def _validate_smtp_config(self) -> bool:
        required_keys = ("SMTP_HOST", "SMTP_PORT", "SMTP_MAIL_FROM")
        missing = [k for k in required_keys if not current_app.config.get(k)]
        if missing:
            logger.error(
                "Configuración SMTP incompleta para recuperación de contraseña (faltan %s)",
                ", ".join(missing),
            )
            return False
        return True
    ...
    def _send_reset_link(self, user) -> None:
        if not self._password_reset_email_enabled():
            logger.warning(
                "Envío de correo de recuperación desactivado por configuración."
            )
            return
        ...
        if not user.email:
            logger.warning(
                "El usuario %s no tiene correo registrado; no se envió enlace de recuperación.",
                user.username,
            )
            return
        ...
        reset_url = url_for(
            "PasswordResetView.reset_password", token=token, _external=True
        )
        ...
        try:
            ...
            logger.error(
                "Fallo envío SMTP en recuperación: host=%s port=%s starttls=%s ssl=%s ssl_server_auth=%s user=%s",
                current_app.config.get("SMTP_HOST"),
                current_app.config.get("SMTP_PORT"),
                current_app.config.get("SMTP_STARTTLS"),
                current_app.config.get("SMTP_SSL"),
                current_app.config.get("SMTP_SSL_SERVER_AUTH"),
            ...
            if isinstance(exc, TypeError) and "linesep" in str(exc):
                ...
                try:
                    ...
                    if current_app.config.get("PASSWORD_RESET_RAISE_EMAIL_ERRORS"):
                        raise
                    return
