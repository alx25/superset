...
def pivot_table_rx1(
    ...
    - Visible formula metrics are included in the pivot output.
    - The original row order from the DB (sort rows by) is preserved after pivoting.
    """
    ...
    # Collect visible (non-hidden) formula metric labels present in the dataframe
    ...
    # Capture the original row order BEFORE pivoting (respects "Sort rows by").
    # pivot_table sorts the index alphabetically by default, losing the DB ordering.
    original_row_order: list[Any] = []
    if rows:
        seen: set[Any] = set()
        if len(rows) == 1:
            # Single row dimension → index values are scalars
            for val in df[rows[0]]:
                if val not in seen:
                    seen.add(val)
                    original_row_order.append(val)
        else:
            # Multiple row dimensions → index values are tuples
