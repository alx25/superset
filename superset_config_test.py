...
def FLASK_APP_MUTATOR(app):  # type: ignore[misc]
    # Rompe un import circular latente de Superset 6.1.0 (también en el tag
    # oficial): `sync_config_to_db()` corre justo después de este mutator y
    # antes de init_views(), e importa superset.commands.base en frío →
    # commands.utils → daos.tag → … → views.base → db_engine_specs.gsheets →
    # databases.schemas → commands.database.uploaders.base → commands.base
    # (a medio inicializar). Resultado: "Failed to sync configuration to
    # database" en cada arranque y nunca se sembraban los temas de sistema
    # ni los listeners de TAGGING_SYSTEM. Cargar superset.views primero es el
    # único punto de entrada que completa el ciclo sin error (verificado
    # 2026-09-23, Registro de cambios entrada 47).
    import superset.views  # noqa: F401
...
MCP_TOOL_SEARCH_CONFIG = {
    **_MCP_TS,
    "always_visible": [
        ...
        "extensions.irex.irex-mcp-tools.irex.rank_partitions",
